...
def average_pixel(image_path):
    ...
    with Image.open(image_path) as img:
        ...
        average_color = pixel_data.mean(axis=(0, 1))  # Average over height and width
...
def execute(xml_filename):
            ...
            for geom in geoms:
                ...
                if geom.get("type") == "mesh":
                    ...
                    if material_file is not None and obj_file is not None:
                        obj_to_png[obj_file] = {"file": material_file, "name": body_name}
                    else:
                        # print(f"cannot find {material}, {mesh}: getting {material_file}, {obj_file}")
                        if material_file is not None:
                            obj_to_png[mesh] = {"file": material_file, "name": body_name}
                    ...
                    if material_file is not None:
                        ...
                        geom.set("rgba", f"{avg_pixel[0] / 255:.3f} {avg_pixel[1] / 255:.3f} {avg_pixel[2] / 255:.3f} {avg_pixel[3] / 255:.3f}")
    ...
    for mesh in mesh_tags:
        ...
        if current_file in obj_to_update:
            ...
            new_file_name = obj_to_png[current_file]["name"] + "_" + current_file.split("/")[-1]
...
# execute("../robocasa/modified_baseline_model_no_doors.xml")
